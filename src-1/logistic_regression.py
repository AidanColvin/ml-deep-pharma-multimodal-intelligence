...
def main():
    ...
    sev_model = Pipeline([('tfidf', TfidfVectorizer(max_features=5000)), ('lr', LogisticRegression(max_iter=1000))])
    bin_model = Pipeline([('tfidf', TfidfVectorizer(max_features=5000)), ('lr', MultiOutputClassifier(LogisticRegression(max_iter=1000)))])
    prr_model = Pipeline([('tfidf', TfidfVectorizer(max_features=5000)), ('ridge', MultiOutputRegressor(Ridge()))])
    ...
    print(f"RESULT_LG_SEV: {sev_acc:.4f}")
    print(f"RESULT_LG_BIN: {bin_acc:.4f}")
